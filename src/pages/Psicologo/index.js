    ...
    async function loadAvailabilities() {
      ...
      const formatData = availabilitiesData.map(availability => {
        ...
        if (fromMinutes > 0 || toMinutes > 0) {
          ...
          return {
            ...availability,
            from: `${fromHours < 10 ? '0'+fromHours : fromHours}:${fromMinutes < 10 ? '0'+fromMinutes : fromMinutes}`,
            to: `${toHours < 10 ? '0'+toHours : toHours}:${toMinutes < 10 ? '0'+toMinutes : toMinutes}`
          }
        }
    ...
    <div className="painel-psicologo">
      ...
      <div className="call container">
        ...
        <div className="accept-decline">
          <button className="btn-primary btn-accept">Aceitar</button>
          <button className="btn-secondary btn-decline">Recusar</button>
        </div>
      ...
      <div className="edit-availability">
        <div className="small-container">
          ...
          <div>
            <h5>Meus horários</h5>
            { myAvailabilities && myAvailabilities.map(item => (
                <div className='row hours' key={item.id}>
                  <p>
                    {
                      item.week_day === 1 ? 'Segunda-feira'
                      : item.week_day === 2 ? 'Terça-feira'
                      : item.week_day === 3 ? 'Quarta-feira'
                      : item.week_day === 4 ? 'Quinta-feira'
                      : item.week_day === 5 ? 'Sexta-feira' : ''
                    }
                  </p>
                  <p>das</p>
                  <p>{item.from}</p>
                  ...
                  <div className="del-availability" onClick={() => deleteAvailability(item.id)}>
                    <p>Excluir</p>
                  </div>
                </div>
              ))
         ...
         {availabilities && availabilities.map((availability, index) => (
            ...
            <div key={index} className="row">
              <div className="col-md-6">
                ...
                onChange={e => setItemAvailability(index, 'week_day', e.target.value)}
                >
              ...
              <div className="col-md-3">
                <input
                  ...
                  onChange={e => setItemAvailability(index, 'from', e.target.value)}
                />
              </div>
              <div className="col-md-3">
                <input
                  ...
                  onChange={e => setItemAvailability(index, 'to', e.target.value)}
                />
              </div>
            </div>
